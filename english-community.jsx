    ...
    <div>
      ...
      <div className="ec-card" style={{ padding: 26, marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 14 }}>
          <div><span className="ec-label">Kata Sandi Lama</span><input type="password" className="ec-input" value={pwForm.lama} onChange={(e) => setPwForm({ ...pwForm, lama: e.target.value })} /></div>
          <div><span className="ec-label">Kata Sandi Baru</span><input type="password" className="ec-input" value={pwForm.baru} onChange={(e) => setPwForm({ ...pwForm, baru: e.target.value })} /></div>
          <div><span className="ec-label">Konfirmasi Baru</span><input type="password" className="ec-input" value={pwForm.konfirmasi} onChange={(e) => setPwForm({ ...pwForm, konfirmasi: e.target.value })} /></div>
        </div>
        {pwMsg.text && <div style={{ color: pwMsg.type === "ok" ? "#7fd9b0" : "#e19a9a", fontSize: 12.5, marginBottom: 12 }}>{pwMsg.text}</div>}
        <button className="ec-btn solid" onClick={changePassword}><Save size={13} /> Perbarui Kata Sandi</button>
      </div>

      <div className="ec-card" style={{ padding: 26 }}>
        <div className="ec-serif" style={{ fontSize: 18, color: "var(--gold-soft)", marginBottom: 10 }}>Kelola Konten Lainnya</div>
        <p style={{ color: "var(--text-lo)", fontSize: 13, lineHeight: 1.8 }}>
          Struktur organisasi, media sosial, berita, data anggota (termasuk impor/ekspor Excel), tutor,
          serta rekap nilai/kas/kehadiran (termasuk impor/ekspor Excel) kini dapat langsung dikelola
          melalui tombol yang muncul di masing-masing halaman navigasi selama mode admin aktif.
        </p>
      </div>
    </div>
  );
...
function EmptyState({ text }) {
  return <div className="ec-card" style={{ padding: 30, textAlign: "center", color: "var(--text-lo)", fontSize: 13.5 }}>{text}</div>;
}
