...
const TRANSLATIONS = {
  id: {
    ...
    nav_socmed: "Media Sosial",
    nav_berita: "Berita & Pengumuman",
    nav_anggota: "Data Anggota",
    nav_daftarulang: "Daftar Ulang",
    nav_daftareskul: "Daftar Ekstrakulikuler",
    nav_evoting: "E-Voting",
    nav_rekap: "Rekap Nilai & Kas",
    nav_tutor: "Tutor Pengajar",
    nav_admin: "Panel Admin"
  },
  en: {
    langBtn: "EN",
    totalMembers: "Total Members",
    tutors: "Teaching Tutors",
    established: "Established In",
    vision: "Vision",
    mission: "Mission",
    latestHighlight: "Latest Highlights",
    seeAll: "See All",
    readMore: "Read more",
    nav_beranda: "Dashboard",
    nav_struktur: "Org. Structure",
    nav_socmed: "Social Media",
    nav_berita: "News & Announcements",
    nav_anggota: "Member Data",
    nav_daftarulang: "Re-registration",
    nav_daftareskul: "Join Club",
    nav_evoting: "E-Voting",
    nav_rekap: "Grades & Cash",
    nav_tutor: "Tutors",
...
const NAV_ITEMS = [
  ...
  { key: "daftareskul", label: "Daftar Ekstrakulikuler", icon: ListChecks },
  { key: "evoting", label: "E-Voting", icon: Vote },
      ...
      <div className="ec-shell">
        ...
        <aside className={`ec-sidebar${sidebarOpen ? " open" : ""}`}>
          ...
          {NAV_ITEMS.map((item) => {
            ...
            const labelText = t[`nav_${item.key}`] || item.label;
            return (
        ...
        <main className="ec-main" style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 16 }}>
          <header className="ec-header-glass">
            ...
            {tab !== "beranda" && (
              <button className="ec-btn ghost" onClick={() => goTo("beranda")} style={{ flexShrink: 0 }}>
                <ArrowLeft size={13} /> {t.nav_beranda}
              </button>
            )}
            <div className="ec-serif" style={{ fontSize: 21, color: "var(--text-hi)" }}>{t[`nav_${activeItem?.key}`] || activeItem?.label}</div>
          ...
          <ErrorBoundary>
            {tab === "beranda" && <Beranda data={data} goTo={goTo} lang={lang} setLang={setLang} t={t} />}
...
function Beranda({ data, goTo, lang, setLang, t }) {
  ...
  return (
    <div>
      <div className="ec-card" style={{ padding: "44px 36px", position: "relative", overflow: "hidden", marginBottom: 20 }}>
        ...
        <div style={{ position: "relative", display: "flex", gap: 32, alignItems: "flex-start", flexWrap: "wrap" }}>
          <div style={{ flex: "1 1 380px" }}>
            ...
            <span className="ec-badge" style={{ marginBottom: 16 }}>Ekstrakurikuler · {t.established} {data.profil.berdiri}</span>
            <h1 className="ec-serif" style={{ fontSize: 42, lineHeight: 1.15, margin: "14px 0 10px", color: "var(--text-hi)" }}>{data.profil.nama}</h1>
            <p style={{ color: "var(--gold-soft)", fontSize: 15, marginBottom: 18, letterSpacing: ".3px", fontWeight: 600 }}>{data.profil.tagline}</p>
            <p style={{ color: "var(--text-lo)", fontSize: 14.5, lineHeight: 1.85, maxWidth: 560 }}>{data.profil.deskripsi}</p>
          </div>
          
          <div 
            className="ec-flashcard clickable" 
      ...
      <div className="ec-card" style={{ display: "flex", flexWrap: "wrap", marginBottom: 20 }}>
        <div className="ec-stat"><span className="ec-label">{t.totalMembers}</span><span className="ec-serif" style={{ fontSize: 26, color: "var(--gold-soft)" }}>{totalAnggota}</span></div>
        <div className="ec-stat"><span className="ec-label">{t.tutors}</span><span className="ec-serif" style={{ fontSize: 26, color: "var(--gold-soft)" }}>{data.tutors.length}</span></div>
        <div className="ec-stat"><span className="ec-label">{t.established}</span><span className="ec-serif" style={{ fontSize: 26, color: "var(--gold-soft)" }}>{data.profil.berdiri}</span></div>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: 20, marginBottom: 20 }}>
        <div className="ec-card" style={{ padding: 28 }}>
          <div className="ec-serif" style={{ fontSize: 20, color: "var(--gold-soft)", marginBottom: 12 }}>{t.vision}</div>
          <p style={{ color: "var(--text-lo)", fontSize: 14, lineHeight: 1.8 }}>{data.profil.visi}</p>
        </div>
        <div className="ec-card" style={{ padding: 28 }}>
          <div className="ec-serif" style={{ fontSize: 20, color: "var(--gold-soft)", marginBottom: 12 }}>{t.mission}</div>
      ...
      {latest.length > 0 && (
        <div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
            <div className="ec-serif" style={{ fontSize: 20, color: "var(--gold-soft)" }}>{t.latestHighlight}</div>
            <button className="ec-btn ghost" onClick={() => goTo("berita")}>{t.seeAll} <ArrowUpRight size={13} /></button>
          </div>
...
/* ---------------------------------------------------------
   BERITA (list + detail)
--------------------------------------------------------- */
const EMPTY_BERITA_FORM = { judul: "", isi: "", tanggal: new Date().toISOString().slice(0, 10), gambarList: [""] };
          ...
          <div key={b.id} className="ec-card clickable" style={{ padding: 22, position: "relative", overflow: "hidden" }} onClick={() => onOpen(b.id)}>
            {b.gambarList && b.gambarList.length > 0 && (
              b.gambarList.length === 1 ? (
                <img src={b.gambarList[0]} alt={b.judul} style={{ width: "100%", maxHeight: 220, objectFit: "cover", borderRadius: 14, marginBottom: 16 }} />
              ) : (
