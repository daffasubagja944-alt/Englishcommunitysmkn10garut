  ...
  async function handleFileUpload(e) {
    ...
    try {
      ...
      for (const file of files) {
        try { compressed.push(await compressImageFile(file)); }
        catch { /* lewati file yang gagal diproses */ }
      }
      setForm((f) => {
        const existing = f.gambarList.filter((g) => g.trim());
        return { ...f, gambarList: [...existing, ...compressed] };
      });
    } finally {
      setUploading(false);
                ...
                <div>
                  ...
                  {form.gambarList.map((g, i) => (
                    <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}>
                      {g && g.trim() && (
                        <img src={g} alt="" style={{ width: 40, height: 40, objectFit: "cover", borderRadius: 8, flexShrink: 0 }} />
                      )}
                      <input className="ec-input" placeholder="https://… (opsional, atau upload di atas)" value={g.startsWith("data:") ? "[foto ter-upload]" : g} readOnly={g.startsWith("data:")} onChange={(e) => updateFoto(i, e.target.value)} />
