...
const GlobalStyle = () => (
  <style>{`
    ...
    * { box-sizing: border-box; }
    ...
    /* --- Liquid-glass horizontal scroll helpers for tables on mobile --- */
    .ec-table-wrap { position: relative; }
    .ec-table-wrap::before, .ec-table-wrap::after {
      content: ""; position: absolute; top: 0; bottom: 0; width: 26px; z-index: 4;
      pointer-events: none; opacity: 1; transition: opacity .25s ease;
    }
    .ec-table-wrap::before { left: 0; background: linear-gradient(90deg, rgba(9,13,23,.85), transparent); }
    .ec-table-wrap::after { right: 0; background: linear-gradient(270deg, rgba(9,13,23,.85), transparent); }
    .ec-table-wrap.at-start::before { opacity: 0; }
    .ec-table-wrap.at-end::after { opacity: 0; }

    .ec-table th.ec-sticky-col, .ec-table td.ec-sticky-col {
      position: sticky; right: 0; z-index: 3;
      background: rgba(9,13,23,.78);
      backdrop-filter: blur(20px) saturate(180%);
      -webkit-backdrop-filter: blur(20px) saturate(180%);
      box-shadow: -10px 0 18px -10px rgba(0,0,0,.5);
    }
    .ec-table tr:hover td.ec-sticky-col { background: rgba(18,25,40,.85); }

    .ec-hscroll-track {
      height: 5px; border-radius: 999px; margin: 12px 16px 14px;
      background: rgba(255,255,255,.05); border: 1px solid rgba(255,255,255,.06);
      position: relative; overflow: hidden;
    }
    .ec-hscroll-thumb {
      position: absolute; top: -1px; bottom: -1px; border-radius: 999px;
      background: linear-gradient(90deg, rgba(255,255,255,.5), rgba(147,182,238,.9));
      backdrop-filter: blur(6px); -webkit-backdrop-filter: blur(6px);
      box-shadow: 0 0 10px rgba(147,182,238,.5);
...
function Anggota({ data, persist, isAdmin }) {
  ...
  const [form, setForm] = useState({ nama: "", nisn: "", jurusan: jurusanOpsi[0] || "", status: "Aktif", eskul: "", kontak: "" });
  const [editIdx, setEditIdx] = useState(null);
  const [editForm, setEditForm] = useState(null);
  const scrollRef = useRef(null);
  const [scrollInfo, setScrollInfo] = useState({ thumbWidth: 100, thumbLeft: 0, atStart: true, atEnd: true });

  function readScroll(el) {
    if (!el) return;
    const scrollable = el.scrollWidth > el.clientWidth + 1;
    if (!scrollable) { setScrollInfo({ thumbWidth: 100, thumbLeft: 0, atStart: true, atEnd: true }); return; }
    const thumbWidth = Math.max(15, (el.clientWidth / el.scrollWidth) * 100);
    const maxScroll = el.scrollWidth - el.clientWidth;
    const thumbLeft = maxScroll > 0 ? (el.scrollLeft / maxScroll) * (100 - thumbWidth) : 0;
    setScrollInfo({
      thumbWidth, thumbLeft,
      atStart: el.scrollLeft <= 2,
      atEnd: el.scrollLeft >= maxScroll - 2,
    });
  }
  function handleTableScroll(e) { readScroll(e.target); }
  useEffect(() => { readScroll(scrollRef.current); }, [kelas, isAdmin, data.members[kelas].length, editIdx]);

  function tambah() {
    if (!form.nama.trim()) return;
    persist({ ...data, members: { ...data.members, [kelas]: [...data.members[kelas], { ...form }] } });
    setForm({ nama: "", nisn: "", jurusan: jurusanOpsi[0] || "", status: "Aktif", eskul: "", kontak: "" });
  }
  function hapus(idx) {
  ...
  function handleImport(rows) {
    ...
    rows.forEach((r) => {
      ...
      const entry = {
        ...
        kontak: (r.kontak || r.wa || r.whatsapp || r.no_wa || r.telepon || "").toString(),
      };
  ...
  return (
    <div>
      ...
      {isAdmin && (
        <div className="ec-card" style={{ padding: 18, marginBottom: 18, display: "flex", gap: 14, flexWrap: "wrap", alignItems: "flex-end" }}>
          ...
          <div style={{ minWidth: 140 }}>
            <span className="ec-label">No. WhatsApp</span>
            <input className="ec-input" value={form.kontak} onChange={(e) => setForm({ ...form, kontak: e.target.value })} />
          </div>
          <button className="ec-btn solid" onClick={tambah}><Plus size={13} /> Tambah</button>
          <ImportButton label="Impor dari Excel" onRows={handleImport} hint={`Wajib: nama. Opsional: nisn, kelas, jurusan, status, eskul, kontak. Kalau file punya kolom kelas, baris otomatis masuk ke kelas itu — kalau tidak, masuk ke Kelas ${kelas} yang sedang dipilih.`} />
        </div>
      )}
      ...
      <div className="ec-card" style={{ overflow: "hidden" }}>
        <div className={`ec-table-wrap ${scrollInfo.atStart ? "at-start" : ""} ${scrollInfo.atEnd ? "at-end" : ""}`}>
        <div className="ec-scrollbar" ref={scrollRef} onScroll={handleTableScroll} style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
        <table className="ec-table" style={{ minWidth: 640 }}>
          <thead><tr><th style={{ width: 40 }}>#</th><th>Nama</th><th>NISN</th><th>Jurusan</th><th>Status</th><th>Eskul</th>{isAdmin && <th>No. WA</th>}{isAdmin && <th className="ec-sticky-col" style={{ width: 90 }}></th>}</tr></thead>
          <tbody>
            {data.members[kelas].length === 0 && <tr><td colSpan={8} style={{ color: "var(--text-lo)", textAlign: "center", padding: 26 }}>Belum ada anggota kelas {kelas}.</td></tr>}
            {data.members[kelas].map((m, i) => {
              ...
              if (isAdmin && editIdx === i) {
                return (
                  <tr key={i} style={{ background: "rgba(140,170,255,.05)" }}>
                    ...
                    <td><input className="ec-input" style={{ padding: "6px 8px", minWidth: 110 }} value={editForm.kontak || ""} onChange={(e) => setEditForm({ ...editForm, kontak: e.target.value })} /></td>
                    <td className="ec-sticky-col">
              ...
              return (
                <tr key={i}>
                  ...
                  <td>{m.eskul || "-"}</td>
                  {isAdmin && <td>{m.kontak ? <a href={waLink(m.kontak, `Halo ${m.nama}`)} target="_blank" rel="noreferrer" style={{ color: "var(--gold-soft)" }}>{m.kontak}</a> : "-"}</td>}
                  {isAdmin && (
                    <td className="ec-sticky-col">
                      <div style={{ display: "flex", gap: 6 }}>
        ...
        {scrollInfo.thumbWidth < 99.5 && (
          <div className="ec-hscroll-track">
            <div className="ec-hscroll-thumb" style={{ width: `${scrollInfo.thumbWidth}%`, left: `${scrollInfo.thumbLeft}%` }} />
          </div>
        )}
      </div>
    </div>
