...
const DEFAULT_DATA = {
  ...
  profil: {
    ...
    berdiri: "2019",
    jurusanList: ["RPL", "TKJ", "MM", "AKL", "OTKP", "BDP"],
  },
  struktur: {
    ketua: "Nama Ketua",
  ...
  members: {
    X: [{ nama: "Contoh Nama", nisn: "0024001", jurusan: "RPL", status: "Aktif" }],
    XI: [{ nama: "Contoh Nama", nisn: "0023001", jurusan: "RPL", status: "Aktif" }],
    XII: [{ nama: "Contoh Nama", nisn: "0022001", jurusan: "RPL", status: "Aktif" }],
  },
  daftarUlang: [],
  rekap: {
          ...
          return (
            <div key={it.key} className={`ec-card${!edit ? " clickable" : ""}`} style={{ padding: 24 }}
              ...
              {edit ? (
                ...
                <div style={{ color: "var(--text-lo)", fontSize: 14 }}>
                  {data.socmed[it.key] ? `${it.prefix}${data.socmed[it.key]}` : "Belum diatur"}
                </div>
              )}
            </div>
          );
...
function Berita({ data, persist, isAdmin, onOpen }) {
  ...
  const [form, setForm] = useState({ judul: "", isi: "", tanggal: new Date().toISOString().slice(0, 10), gambar: "" });

  function tambah() {
    if (!form.judul.trim() || !form.isi.trim()) return;
    persist({ ...data, berita: [{ id: uid(), ...form }, ...data.berita] });
    setForm({ judul: "", isi: "", tanggal: new Date().toISOString().slice(0, 10), gambar: "" });
    setShowForm(false);
  }
          ...
          {!showForm ? <button className="ec-btn solid" onClick={() => setShowForm(true)}><Plus size={14} /> Tambah Berita</button> : (
            <div className="ec-card" style={{ padding: 22, marginBottom: 20 }}>
              <div style={{ display: "grid", gap: 12 }}>
                ...
                <div><span className="ec-label"><ImageIcon size={11} style={{ verticalAlign: "-2px", marginRight: 4 }} />URL Poster / Gambar (opsional)</span><input className="ec-input" placeholder="https://…" value={form.gambar} onChange={(e) => setForm({ ...form, gambar: e.target.value })} /></div>
      ...
      <div style={{ display: "grid", gap: 14 }}>
        ...
        {data.berita.slice().sort((a, b) => (a.tanggal < b.tanggal ? 1 : -1)).map((b) => (
          <div key={b.id} className="ec-card clickable" style={{ padding: 22, position: "relative", overflow: "hidden" }} onClick={() => onOpen(b.id)}>
            {b.gambar && (
              <img src={b.gambar} alt={b.judul} style={{ width: "100%", maxHeight: 220, objectFit: "cover", borderRadius: 14, marginBottom: 16 }} />
            )}
  ...
  return (
    <div>
      ...
      <div className="ec-card" style={{ padding: 36, maxWidth: 720, overflow: "hidden" }}>
        {b.gambar && (
          <img src={b.gambar} alt={b.judul} style={{ width: "100%", maxHeight: 380, objectFit: "cover", borderRadius: 16, marginBottom: 22 }} />
        )}
