    ...
    <div>
      ...
      <div className="ec-card" style={{ overflow: "hidden" }}>
        <table className="ec-table">
          <thead><tr><th style={{ width: 40 }}>#</th><th>Nama</th><th>NISN</th><th>Jurusan</th><th>Status</th><th>Eskul</th><th>No. WA</th>{isAdmin && <th style={{ width: 90 }}></th>}</tr></thead>
          <tbody>
            ...
            {data.members[kelas].map((m, i) => {
              ...
              return (
                <tr key={i}>
                  ...
                  <td>{m.eskul || "-"}</td>
                  <td>{m.kontak ? <a href={waLink(m.kontak, `Halo ${m.nama}`)} target="_blank" rel="noreferrer" style={{ color: "var(--gold-soft)" }}>{m.kontak}</a> : "-"}</td>
                  {isAdmin && (
                    <td>
                      <div style={{ display: "flex", gap: 6 }}>
