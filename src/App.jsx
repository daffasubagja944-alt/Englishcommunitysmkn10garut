...
--------------------------------------------------------- */
const DEFAULT_DATA = {
  auth: { password: "admin123", recoveryCode: "GANTI-KODE-INI" },
  profil: {
    logo: "EC",
    logoUrl: "",
  ...
  struktur: {
    ketua: "Febrian Rahmat Saputra",
    wakilKetua: "Dewa Ridho Nagano",
    sekretaris: "Desi Heryani",
    wakilSekretaris: "Azka Naufal Ahmad",
    bendahara: "Nayla Sifa Agustina",
    wakilBendahara: "Messa Alfarizi",
    divisi: [
      { nama: "Divisi PDD", ketua: "M Hasbi", wakil: "Fadil", anggota: ["Anisa"] },
      { nama: "Divisi Humas", ketua: "Putra Pangestu", wakil: "Melisa Malaika", anggota: ["Ai Resti Diani Putri", "Yordhan Aryadi", "Zaydan Al-Fatih"] },
      { nama: "Divisi Koordinasi Acara", ketua: "Ahkam Alqi Alifa", wakil: "Alfin Fauji", anggota: ["Syerina", "Cantika", "Selvi", "M. Rezma", "Imam Fahlen", "M. Rizki Ramadhan", "Dival Nugraha"] },
    ],
  },
  socmed: {
  ...
  members: {
    X: [{ nama: "Contoh Nama", nisn: "0024001", jurusan: "LPS", status: "Aktif", eskul: "English Community", kontak: "" }],
    XI: [{ nama: "Contoh Nama", nisn: "0023001", jurusan: "LPS", status: "Aktif", eskul: "English Community", kontak: "" }],
    XII: [{ nama: "Contoh Nama", nisn: "0022001", jurusan: "LPS", status: "Aktif", eskul: "English Community", kontak: "" }],
  },
  daftarUlang: [],
...
const GlobalStyle = () => (
  <style>{`
    ...
    * { box-sizing: border-box; }
    ...
    .ec-footer-top { display: grid; grid-template-columns: 1.4fr 1fr 1fr 1fr; gap: 28px; }
    .ec-footer-brand-desc { color: var(--text-lo); font-size: 12.5px; line-height: 1.8; margin-top: 10px; max-width: 320px; }
    .ec-footer-heading { font-size: 11px; text-transform: uppercase; letter-spacing: .8px; font-weight: 700; color: var(--gold-soft); margin-bottom: 14px; }
    .ec-footer-links { display: flex; flex-direction: column; gap: 10px; }
    .ec-footer-link { background: none; border: none; padding: 0; text-align: left; font: inherit; cursor: pointer; color: var(--text-lo); font-size: 12.5px; transition: color .25s ease; }
    .ec-footer-link:hover { color: var(--text-hi); }
    .ec-footer-social { display: flex; gap: 10px; margin-top: 4px; }
    .ec-footer-social a {
      width: 32px; height: 32px; border-radius: 50%;
      display: flex; align-items: center; justify-content: center;
      background: rgba(255,255,255,.04); border: 1px solid rgba(255,255,255,.08);
      color: var(--text-lo); transition: color .25s ease, border-color .25s ease;
    }
    .ec-footer-social a:hover { color: var(--gold-soft); border-color: rgba(147,182,238,.4); }
    .ec-footer-bottom {
    ...
    @media (max-width: 860px) {
      ...
      .ec-footer { border-radius: 0; padding: 26px 20px 18px; }
      .ec-footer-top { grid-template-columns: 1fr 1fr; gap: 24px 16px; }
...
// Mengompres & mengecilkan foto di browser (max lebar 1280px, kualitas 75%)
// sebelum diubah jadi base64, supaya data yang tersimpan tetap ringan & permanen
// (tidak numpang ke link luar seperti ImgBB yang bisa expired).
...
// --- Penyimpanan foto terpisah dari data utama -------------------------
// Supaya paket data utama (ec-site-data) tetap kecil & ringan di semua
// halaman, setiap foto yang di-upload disimpan di key-nya sendiri-sendiri
// (bukan ditempel langsung ke dalam data berita), dan baru dimuat pas
// beneran dibutuhkan untuk ditampilkan (lazy-load).
function genAssetKey() {
  return `foto-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
}
async function saveImageAsset(dataUri) {
  const key = genAssetKey();
...
async function deleteImageAsset(ref) {
  try { await storage.delete(ref.slice("asset:".length)); } catch { /* biarkan, bukan fatal */ }
}
function isAssetRef(v) { return typeof v === "string" && v.startsWith("asset:"); }

// Komponen img yang otomatis "resolve" foto: kalau src berupa link biasa
// atau base64 lama, tampil langsung; kalau berupa referensi asset:...,
// diambil dulu dari penyimpanan terpisah pas komponennya dirender.
function SmartImg({ src, style, alt, className }) {
  const [resolved, setResolved] = useState(() => (isAssetRef(src) ? null : src));
  useEffect(() => {
    let alive = true;
    if (isAssetRef(src)) {
      setResolved(null);
      loadImageAsset(src).then((v) => { if (alive) setResolved(v); });
    } else {
      setResolved(src);
    }
    return () => { alive = false; };
  ...
  if (!resolved) {
    return <div className={className} style={{ ...style, background: "rgba(255,255,255,.05)" }} />;
  }
  return <img src={resolved} alt={alt || ""} className={className} style={style} loading="lazy" decoding="async" />;
}
...
// Saat siswa/siswi daftar ulang, pindahkan/perbarui datanya di Data Anggota:
// dicari dulu di kelas asal berdasarkan nama, lalu dipindah/diperbarui ke kelas baru
// dengan jurusan terbaru & status Aktif. Kalau belum ada datanya sama sekali, tetap dibuatkan baru.
function applyDaftarUlangToMembers(members, form) {
  const namaLower = form.nama.trim().toLowerCase();
  const next = { ...members };
  const asalList = next[form.kelasAsal] || [];
  const existing = asalList.find((m) => (m.nama || "").trim().toLowerCase() === namaLower);
  next[form.kelasAsal] = asalList.filter((m) => (m.nama || "").trim().toLowerCase() !== namaLower);
  ...
  const baruList = [...(next[form.kelasBaru] || [])];
  const idxBaru = baruList.findIndex((m) => (m.nama || "").trim().toLowerCase() === namaLower);
  if (idxBaru !== -1) baruList[idxBaru] = updatedMember; else baruList.push(updatedMember);
  next[form.kelasBaru] = baruList;
...
/* ---------------------------------------------------------
   FOOTER
--------------------------------------------------------- */
const FOOTER_NAV_KEYS = ["beranda", "struktur", "berita", "anggota", "daftareskul"];

function Footer({ data, goTo, lang }) {
  const t = {
    id: { navigasi: "Navigasi", sosial: "Media Sosial", tentang: "Tentang", jelajahi: "Jelajahi", hak: "Hak cipta dilindungi." },
    en: { navigasi: "Navigation", sosial: "Social Media", tentang: "About", jelajahi: "Explore", hak: "All rights reserved." },
  }[lang];

  const navLinks = NAV_ITEMS.filter((item) => FOOTER_NAV_KEYS.includes(item.key));
  const socials = [
    { key: "instagram", icon: Instagram, handle: data.socmed.instagram },
    { key: "tiktok", icon: Music2, handle: data.socmed.tiktok },
  ].filter((s) => s.handle);
  ...
  return (
    <footer className="ec-footer">
      <div className="ec-footer-top">
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div className="ec-crest" style={{ width: 40, height: 40, fontSize: 15 }}>
              {data.profil.logoUrl ? <img src={data.profil.logoUrl} alt="logo" /> : (data.profil.logo || "EC").slice(0, 3)}
            </div>
            <div className="ec-serif" style={{ fontSize: 17, color: "var(--text-hi)" }}>{data.profil.nama}</div>
          </div>
          <p className="ec-footer-brand-desc">{bi(data.profil.tagline, lang)}</p>
          {socials.length > 0 && (
            <div className="ec-footer-social">
              {socials.map((s) => {
                const Icon = s.icon;
                return (
                  <a key={s.key} href={socialUrl(s.key, s.handle)} target="_blank" rel="noreferrer" aria-label={s.key}>
                    <Icon size={14} />
                  </a>
                );
              })}
        ...
        <div>
          <div className="ec-footer-heading">{t.navigasi}</div>
          <div className="ec-footer-links">
            {navLinks.map((item) => (
              <button key={item.key} className="ec-footer-link" onClick={() => goTo(item.key)}>
                {bi(item.label, lang)}
              </button>
            ))}
          </div>
        ...
        <div>
          <div className="ec-footer-heading">{t.jelajahi}</div>
          <div className="ec-footer-links">
            <button className="ec-footer-link" onClick={() => goTo("tutor")}>{bi({ id: "Tutor Pengajar", en: "Tutors" }, lang)}</button>
            <button className="ec-footer-link" onClick={() => goTo("evoting")}>{bi({ id: "E-Voting", en: "E-Voting" }, lang)}</button>
            <button className="ec-footer-link" onClick={() => goTo("daftarulang")}>{bi({ id: "Daftar Ulang", en: "Re-registration" }, lang)}</button>
          </div>
        </div>

        <div>
          <div className="ec-footer-heading">{t.tentang}</div>
          <div className="ec-footer-links">
            <span style={{ color: "var(--text-lo)", fontSize: 12.5 }}>
              {lang === "id" ? "Berdiri sejak" : "Since"} {data.profil.berdiri}
            </span>
            {data.socmed.gdrive && (
              <a className="ec-footer-link" href={socialUrl("gdrive", data.socmed.gdrive)} target="_blank" rel="noreferrer">
                {lang === "id" ? "Arsip Dokumen" : "Document Archive"}
              </a>
            )}
          </div>
        </div>
      ...
      <div className="ec-footer-bottom">
        <span>© {new Date().getFullYear()} {data.profil.nama}. {t.hak}</span>
        <DevBadge />
      </div>
    </footer>
  );
...
export default function App() {
  ...
  const [isAdmin, setIsAdminState] = useState(() => {
    try { return sessionStorage.getItem("ec-admin") === "1"; } catch { return false; }
  });
  function setIsAdmin(val) {
    setIsAdminState(val);
    try {
      if (val) sessionStorage.setItem("ec-admin", "1");
      else sessionStorage.removeItem("ec-admin");
    } catch {}
  }
    ...
    (async () => {
      try {
        ...
        if (res && res.value) {
          ...
          let merged = mergeWithDefaults(DEFAULT_DATA, loaded);

          // Migrasi foto lama yang masih nempel langsung (base64) di data utama
          // ke penyimpanan terpisah, supaya paket data utama jadi ringan lagi.
          let migrated = false;
          if (Array.isArray(merged.berita) && merged.berita.length) {
            const newBerita = [];
            for (const b of merged.berita) {
              if (Array.isArray(b.gambarList) && b.gambarList.some((g) => typeof g === "string" && g.startsWith("data:"))) {
                const newList = [];
                for (const g of b.gambarList) {
                  if (typeof g === "string" && g.startsWith("data:")) {
                    try { newList.push(await saveImageAsset(g)); migrated = true; }
                    catch { newList.push(g); }
                  } else newList.push(g);
                }
                newBerita.push({ ...b, gambarList: newList });
              } else {
                newBerita.push(b);
              }
            }
            merged = { ...merged, berita: newBerita };
          }

          setData(merged);
          if (migrated || JSON.stringify(merged) !== JSON.stringify(loaded)) {
    ...
    <div className="ec-root">
      ...
      <div className="ec-shell">
        ...
        <main className="ec-main" style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <Footer data={data} goTo={goTo} lang={lang} />
        </main>
      </div>
    </div>
  );
...
function Struktur({ data, persist, isAdmin }) {
  ...
  const inti = [
    { label: "Ketua", key: "ketua", wakilKey: "wakilKetua", wakilLabel: "Wakil Ketua" },
    { label: "Sekretaris I", key: "sekretaris", wakilKey: "wakilSekretaris", wakilLabel: "Sekretaris II" },
    { label: "Bendahara I", key: "bendahara", wakilKey: "wakilBendahara", wakilLabel: "Bendahara II" },
  ];
        ...
        {inti.map((it) => (
          <div key={it.key} className="ec-card" style={{ padding: 22 }}>
            <div style={{ textAlign: "center", marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: ".6px", color: "var(--text-lo)" }}>{it.label.replace(/ I$/, "")}</div>
            </div>
            {["main", "wakil"].map((slot) => {
              const fieldKey = slot === "main" ? it.key : it.wakilKey;
              return (
                <div key={slot} style={{ marginBottom: 10 }}>
                  <span className="ec-label">{slot === "main" ? it.label : it.wakilLabel}</span>
                  {edit ? (
...
function Berita({ data, persist, isAdmin, onOpen, lang }) {
  ...
  function hapusFoto(i) {
    const g = form.gambarList[i];
    if (isAssetRef(g)) deleteImageAsset(g);
    setForm({ ...form, gambarList: form.gambarList.filter((_, idx) => idx !== i) });
  }
    ...
    try {
      ...
      for (const file of files) {
        try {
          ...
          const ref = await saveImageAsset(dataUri);
          refs.push(ref);
        } catch { /* lewati file yang gagal diproses */ }
      }
      setForm((f) => {
        const existing = f.gambarList.filter((g) => g.trim());
        return { ...f, gambarList: [...existing, ...refs] };
      });
    } finally {
      setUploading(false);
          ...
          {!showForm ? <button className="ec-btn solid" onClick={() => bukaForm(null)}><Plus size={14} /> Tambah Berita</button> : (
            <div className="ec-card" style={{ padding: 22, marginBottom: 20 }}>
              <div style={{ display: "grid", gap: 12 }}>
                ...
                <div>
                  <span className="ec-label"><ImageIcon size={11} style={{ verticalAlign: "-2px", marginRight: 4 }} />Foto Berita (bisa lebih dari satu)</span>
                  <input ref={fileInputRef} type="file" accept="image/*" multiple style={{ display: "none" }} onChange={handleFileUpload} />
                  <button type="button" className="ec-btn ghost" disabled={uploading} onClick={() => fileInputRef.current && fileInputRef.current.click()}>
                    <Upload size={12} /> {uploading ? "Memproses foto…" : "Upload dari Perangkat"}
                  </button>
                  <div style={{ fontSize: 11.5, color: "var(--text-lo)", margin: "6px 0 10px" }}>
                    Foto yang di-upload tersimpan permanen langsung di aplikasi (tidak bisa hilang seperti link luar). Atau tempel link foto secara manual di bawah kalau perlu.
                  </div>
                  {form.gambarList.map((g, i) => (
                    <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}>
                      {g && g.trim() && (
                        <SmartImg src={g} alt="" style={{ width: 40, height: 40, objectFit: "cover", borderRadius: 8, flexShrink: 0 }} />
                      )}
                      <input className="ec-input" placeholder="https://… (opsional, atau upload di atas)" value={(isAssetRef(g) || g.startsWith("data:")) ? "[foto ter-upload]" : g} readOnly={isAssetRef(g) || g.startsWith("data:")} onChange={(e) => updateFoto(i, e.target.value)} />
                      {form.gambarList.length > 1 && (
                        <button className="ec-btn danger" style={{ padding: "6px 10px" }} onClick={() => hapusFoto(i)}><Trash2 size={12} /></button>
                      )}
                    </div>
                  ))}
                  <button className="ec-btn ghost" onClick={tambahFoto}><Plus size={12} /> Tambah Kolom Link</button>
                </div>
          ...
          <div key={b.id} className="ec-card clickable" style={{ padding: 22, position: "relative", overflow: "hidden" }} onClick={() => onOpen(b.id)}>
            {b.gambarList && b.gambarList.length > 0 && (
              b.gambarList.length === 1 ? (
                <SmartImg src={b.gambarList[0]} alt={bi(b.judul, lang)} style={{ width: "100%", maxHeight: 220, objectFit: "cover", borderRadius: 14, marginBottom: 16 }} />
              ) : (
                <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(b.gambarList.length, 3)}, 1fr)`, gap: 6, marginBottom: 16 }}>
                  {b.gambarList.slice(0, 3).map((g, i) => (
                    <div key={i} style={{ position: "relative" }}>
                      <SmartImg src={g} alt="" style={{ width: "100%", height: 130, objectFit: "cover", borderRadius: 12 }} />
      ...
      <div className="ec-card" style={{ padding: 36, maxWidth: 720, overflow: "hidden" }}>
        {fotos.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: fotos.length === 1 ? "1fr" : "1fr 1fr", gap: 10, marginBottom: 22 }}>
            {fotos.map((g, i) => (
              <SmartImg key={i} src={g} alt="" style={{ width: "100%", maxHeight: fotos.length === 1 ? 380 : 220, objectFit: "cover", borderRadius: 16 }} />
            ))}
          </div>
        )}
...
function Anggota({ data, persist, isAdmin }) {
  ...
  const [form, setForm] = useState({ nama: "", nisn: "", jurusan: jurusanOpsi[0] || "", status: "Aktif", eskul: "" });
  ...
  function tambah() {
    ...
    setForm({ nama: "", nisn: "", jurusan: jurusanOpsi[0] || "", status: "Aktif", eskul: "" });
  }
  function hapus(idx) {
    if (!window.confirm("Yakin mau menghapus anggota ini?")) return;
    persist({ ...data, members: { ...data.members, [kelas]: data.members[kelas].filter((_, i) => i !== idx) } });
  }
  function mulaiEdit(idx) { setEditIdx(idx); setEditForm({ ...data.members[kelas][idx] }); }
  function batalEdit() { setEditIdx(null); setEditForm(null); }
  function simpanEdit(idx) {
    if (!editForm.nama.trim()) return;
    const next = [...data.members[kelas]];
    next[idx] = { ...editForm };
    persist({ ...data, members: { ...data.members, [kelas]: next } });
    setEditIdx(null); setEditForm(null);
  }

  // Deteksi nama ganda di kelas yang sama biar kelihatan kalau ada duplikat
  const namaCount = {};
  data.members[kelas].forEach((m) => {
    const k = (m.nama || "").trim().toLowerCase();
    if (k) namaCount[k] = (namaCount[k] || 0) + 1;
  });

  function handleImport(rows) {
    // Hanya kolom "nama" yang wajib. Kalau file punya kolom nisn/kelas/jurusan/status,
    // langsung dipakai; kalau cuma nama, sisanya dikosongkan untuk diisi manual nanti.
    const kelasValid = ["X", "XI", "XII"];
    const grouped = {};
    rows.forEach((r) => {
      const nama = (r.nama || r.name || "").toString().trim();
      if (!nama) return;
      const rawKelas = (r.kelas || r.class || r.tingkat || "").toString().trim().toUpperCase();
      const targetKelas = kelasValid.includes(rawKelas) ? rawKelas : kelas;
      const entry = {
        nama,
        nisn: (r.nisn || r.nis || r.no_induk || r.id || "").toString(),
        jurusan: (r.jurusan || "").toString(),
        status: (r.status || "Aktif").toString(),
        eskul: (r.eskul || r.ekstrakurikuler || "").toString(),
      };
      if (!grouped[targetKelas]) grouped[targetKelas] = [];
      grouped[targetKelas].push(entry);
    });
    const kelasTerisi = Object.keys(grouped);
    if (kelasTerisi.length === 0) { alert("Tidak ada baris valid ditemukan. Pastikan ada kolom 'nama'."); return; }
    const nextMembers = { ...data.members };
    kelasTerisi.forEach((k) => { nextMembers[k] = [...(nextMembers[k] || []), ...grouped[k]]; });
    persist({ ...data, members: nextMembers });
  }
      ...
      {isAdmin && (
        <div className="ec-card" style={{ padding: 18, marginBottom: 18, display: "flex", gap: 14, flexWrap: "wrap", alignItems: "flex-end" }}>
          ...
          <div style={{ minWidth: 160 }}>
            <span className="ec-label">Eskul yang Diikuti</span>
            <input className="ec-input" placeholder="mis. English Community, Basket" value={form.eskul} onChange={(e) => setForm({ ...form, eskul: e.target.value })} />
          </div>
          <button className="ec-btn solid" onClick={tambah}><Plus size={13} /> Tambah</button>
          <ImportButton label="Impor dari Excel" onRows={handleImport} hint={`Wajib: nama. Opsional: nisn, kelas, jurusan, status, eskul. Kalau file punya kolom kelas, baris otomatis masuk ke kelas itu — kalau tidak, masuk ke Kelas ${kelas} yang sedang dipilih.`} />
        </div>
      )}

      {isAdmin && Object.values(namaCount).some((c) => c > 1) && (
        <div className="ec-card" style={{ padding: "10px 16px", marginBottom: 14, borderColor: "rgba(225,154,154,.35)", fontSize: 12.5, color: "#e19a9a" }}>
          Ada nama yang muncul lebih dari sekali di Kelas {kelas}. Cek baris bertanda <b>Duplikat</b>, lalu edit atau hapus salah satunya.
        </div>
      )}

      <div className="ec-card" style={{ overflow: "hidden" }}>
        <div className="ec-scrollbar" style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
        <table className="ec-table" style={{ minWidth: 640 }}>
          <thead><tr><th style={{ width: 40 }}>#</th><th>Nama</th><th>NISN</th><th>Jurusan</th><th>Status</th><th>Eskul</th>{isAdmin && <th style={{ width: 90 }}></th>}</tr></thead>
          <tbody>
            {data.members[kelas].length === 0 && <tr><td colSpan={7} style={{ color: "var(--text-lo)", textAlign: "center", padding: 26 }}>Belum ada anggota kelas {kelas}.</td></tr>}
            {data.members[kelas].map((m, i) => {
              const isDup = namaCount[(m.nama || "").trim().toLowerCase()] > 1;
              if (isAdmin && editIdx === i) {
                return (
                  <tr key={i} style={{ background: "rgba(140,170,255,.05)" }}>
                    <td>{i + 1}</td>
                    <td><input className="ec-input" style={{ padding: "6px 8px", minWidth: 110 }} value={editForm.nama} onChange={(e) => setEditForm({ ...editForm, nama: e.target.value })} /></td>
                    <td><input className="ec-input" style={{ padding: "6px 8px", minWidth: 90 }} value={editForm.nisn} onChange={(e) => setEditForm({ ...editForm, nisn: e.target.value })} /></td>
                    <td>
                      <select className="ec-input" style={{ padding: "6px 8px", minWidth: 90 }} value={editForm.jurusan} onChange={(e) => setEditForm({ ...editForm, jurusan: e.target.value })}>
                        {jurusanOpsi.length === 0 && <option value="">-</option>}
                        {jurusanOpsi.map((j) => <option key={j} value={j}>{j}</option>)}
                      </select>
                    </td>
                    <td>
                      <select className="ec-input" style={{ padding: "6px 8px", minWidth: 90 }} value={editForm.status} onChange={(e) => setEditForm({ ...editForm, status: e.target.value })}>
                        <option>Aktif</option><option>Tidak Aktif</option>
                      </select>
                    </td>
                    <td><input className="ec-input" style={{ padding: "6px 8px", minWidth: 110 }} value={editForm.eskul || ""} onChange={(e) => setEditForm({ ...editForm, eskul: e.target.value })} /></td>
                    <td>
                      <div style={{ display: "flex", gap: 6 }}>
                        <button className="ec-btn solid" style={{ padding: "5px 8px" }} onClick={() => simpanEdit(i)}><Check size={12} /></button>
                        <button className="ec-btn ghost" style={{ padding: "5px 8px" }} onClick={batalEdit}><ArrowLeft size={12} /></button>
                      </div>
                    </td>
                  </tr>
                );
              ...
              return (
                <tr key={i}>
                  ...
                  <td>
                    {m.nama}
                    {isDup && <span className="ec-badge" style={{ marginLeft: 8, color: "#e19a9a", borderColor: "rgba(225,154,154,.35)" }}>Duplikat</span>}
                  </td>
                  <td>{m.nisn}</td><td>{m.jurusan}</td>
                  <td><span style={{ color: m.status === "Aktif" ? "#7fd9b0" : "var(--text-lo)" }}>{m.status}</span></td>
                  <td>{m.eskul || "-"}</td>
                  {isAdmin && (
                    <td>
                      <div style={{ display: "flex", gap: 6 }}>
                        <button className="ec-btn ghost" style={{ padding: "5px 8px" }} onClick={() => mulaiEdit(i)}><Pencil size={12} /></button>
                        <button className="ec-btn danger" style={{ padding: "5px 8px" }} onClick={() => hapus(i)}><Trash2 size={12} /></button>
                      </div>
                    </td>
                  )}
                </tr>
...
function DaftarUlang({ data, persist, isAdmin }) {
  ...
  function submit() {
    ...
    const nextMembers = applyDaftarUlangToMembers(data.members, form);
    persist({ ...data, daftarUlang: [entry, ...data.daftarUlang], members: nextMembers });
      ...
      {isAdmin && (
        <div className="ec-card" style={{ padding: 24 }}>
          ...
          <p style={{ color: "var(--text-lo)", fontSize: 11.5, marginBottom: 14, lineHeight: 1.6 }}>
            Situs ini tidak dapat menulis langsung ke Google Sheets tanpa server/API terhubung. Semua data yang masuk tersimpan otomatis di sini — tinggal klik <b>Unduh Excel</b> kapan saja lalu unggah ke Google Sheets bila perlu. Setiap kali ada yang daftar ulang, datanya juga otomatis dipindah/diperbarui di halaman <b>Data Anggota</b> sesuai kelas baru.
          </p>
...
function RekapNilai({ data, persist, isAdmin }) {
  ...
  return (
    <div>
      ...
      <div className="ec-card" style={{ overflow: "hidden" }}>
        <div className="ec-scrollbar" style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
        <table className="ec-table" style={{ minWidth: 480 }}>
...
function AdminPanel({ data, persist, isAdmin, setIsAdmin }) {
  ...
  const [resetForm, setResetForm] = useState({ kode: "", baru: "", konfirmasi: "" });
  ...
  useEffect(() => setRecoveryForm(data.auth.recoveryCode || ""), [data.auth.recoveryCode]);
  ...
  function changePassword() {
    ...
    persist({ ...data, auth: { ...data.auth, password: pwForm.baru } });
    setPwForm({ lama: "", baru: "", konfirmasi: "" });
    setPwMsg({ type: "ok", text: "Kata sandi berhasil diperbarui." });
  }

  function resetWithCode() {
    if (!resetForm.kode || resetForm.kode !== (data.auth.recoveryCode || "")) { setResetMsg({ type: "err", text: "Kode pemulihan salah." }); return; }
    if (resetForm.baru.length < 4) { setResetMsg({ type: "err", text: "Kata sandi baru minimal 4 karakter." }); return; }
    if (resetForm.baru !== resetForm.konfirmasi) { setResetMsg({ type: "err", text: "Konfirmasi kata sandi tidak cocok." }); return; }
    persist({ ...data, auth: { ...data.auth, password: resetForm.baru } });
    setResetForm({ kode: "", baru: "", konfirmasi: "" });
    setResetMsg({ type: "ok", text: "Kata sandi berhasil direset. Silakan masuk dengan kata sandi baru." });
    setShowReset(false);
  }

  function saveRecoveryCode() {
    if (!recoveryForm.trim()) { setRecoveryMsg({ type: "err", text: "Kode pemulihan tidak boleh kosong." }); return; }
    persist({ ...data, auth: { ...data.auth, recoveryCode: recoveryForm.trim() } });
    setRecoveryMsg({ type: "ok", text: "Kode pemulihan berhasil diperbarui." });
  }
      ...
      <div className="ec-card" style={{ padding: 40, maxWidth: 400, textAlign: "center" }}>
        ...
        {!showReset ? (
          <>
            ...
            <button className="ec-btn ghost" style={{ width: "100%", justifyContent: "center", marginTop: 10 }} onClick={() => { setShowReset(true); setResetMsg({ type: "", text: "" }); }}><KeyRound size={13} /> Lupa / Sandi Diubah Orang Lain?</button>
          </>
        ) : (
          <>
            <p style={{ color: "var(--text-lo)", fontSize: 12.5, marginBottom: 18, lineHeight: 1.7 }}>
              Kalau kata sandi admin tiba-tiba berubah (misalnya diutak-atik orang lain), masukkan <b>kode pemulihan</b> rahasia yang sudah kamu simpan untuk mengatur ulang kata sandi tanpa perlu tahu kata sandi yang sedang aktif.
            </p>
            <div style={{ display: "grid", gap: 10, textAlign: "left" }}>
              <div><span className="ec-label">Kode Pemulihan</span><input className="ec-input" value={resetForm.kode} onChange={(e) => setResetForm({ ...resetForm, kode: e.target.value })} /></div>
              <div><span className="ec-label">Kata Sandi Baru</span><input type="password" className="ec-input" value={resetForm.baru} onChange={(e) => setResetForm({ ...resetForm, baru: e.target.value })} /></div>
              <div><span className="ec-label">Konfirmasi Kata Sandi Baru</span><input type="password" className="ec-input" value={resetForm.konfirmasi} onChange={(e) => setResetForm({ ...resetForm, konfirmasi: e.target.value })} /></div>
            </div>
            {resetMsg.text && <div style={{ color: resetMsg.type === "ok" ? "#7fd9b0" : "#e19a9a", fontSize: 12, margin: "12px 0" }}>{resetMsg.text}</div>}
            <button className="ec-btn solid" style={{ width: "100%", justifyContent: "center", marginTop: 6 }} onClick={resetWithCode}><KeyRound size={13} /> Reset Kata Sandi</button>
            <button className="ec-btn ghost" style={{ width: "100%", justifyContent: "center", marginTop: 10 }} onClick={() => setShowReset(false)}><ArrowLeft size={13} /> Kembali ke Login</button>
          </>
        )}
      </div>
    );
  ...
  return (
    <div>
      ...
      <div className="ec-card" style={{ padding: 26, marginBottom: 20 }}>
        ...
        <p style={{ color: "var(--text-lo)", fontSize: 12.5, marginBottom: 16, lineHeight: 1.7 }}>
          Simpan kode ini di tempat aman (jangan disebar). Kalau suatu saat kata sandi admin berubah tanpa sepengetahuanmu, kode ini satu-satunya cara untuk reset kata sandi dari layar login tanpa perlu tahu kata sandi yang sedang aktif.
        </p>
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 10, alignItems: "end" }}>
          <div><span className="ec-label"><KeyRound size={11} style={{ verticalAlign: "-2px", marginRight: 4 }} />Kode Pemulihan</span><input className="ec-input" value={recoveryForm} onChange={(e) => setRecoveryForm(e.target.value)} /></div>
          <button className="ec-btn solid" onClick={saveRecoveryCode}><Save size={13} /> Simpan Kode</button>
        </div>
        {recoveryMsg.text && (
          <div style={{ color: recoveryMsg.type === "ok" ? "#7fd9b0" : "#e19a9a", fontSize: 12.5, marginTop: 12 }}>
            {recoveryMsg.text}
          </div>
        )}
      </div>
