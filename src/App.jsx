  ...
  <style>{`
    ...
    * { box-sizing: border-box; }
    ...
    .ec-card.clickable:hover {
      ...
      box-shadow: 0 20px 46px rgba(0,0,0,.45), 0 0 0 1px rgba(212,175,122,.08), inset 0 1px 0 rgba(255,255,255,.06);
    }
    ...
    .ec-navlink.active {
      background: linear-gradient(135deg, rgba(212,175,122,.14), rgba(255,255,255,.03));
    ...
    .ec-btn.solid:hover { background: var(--gold-soft); border-color: var(--gold-soft); box-shadow: 0 14px 32px rgba(212,175,122,.28); }
    ...
    .ec-input:focus { border-color: var(--gold); background: rgba(255,255,255,.04); box-shadow: 0 0 0 3px rgba(212,175,122,.12); }
    ...
    .ec-orbs { position: fixed; inset: 0; overflow: hidden; z-index: 0; pointer-events: none; background: radial-gradient(ellipse at top, #0a0a0f 0%, #050505 60%); }
    .ec-orb { position: absolute; border-radius: 50%; filter: blur(110px); }
    .ec-orb-a { width: 520px; height: 520px; top: -160px; left: -140px; background: radial-gradient(circle, rgba(212,175,122,.28) 0%, transparent 70%); animation: ecFloat 18s ease-in-out infinite; }
    .ec-orb-b { width: 560px; height: 560px; bottom: -200px; right: -160px; background: radial-gradient(circle, rgba(124,140,255,.22) 0%, transparent 70%); animation: ecFloat 22s ease-in-out infinite reverse; }
    .ec-orb-c { width: 420px; height: 420px; top: 38%; left: 46%; background: radial-gradient(circle, rgba(255,90,160,.14) 0%, transparent 70%); animation: ecFloat 26s ease-in-out infinite; }
    @keyframes ecFloat {
    ...
    .ec-hero-glow {
      ...
      background: radial-gradient(560px 300px at 12% 0%, rgba(212,175,122,.08), transparent 65%);
    }
    ...
    .ec-devbadge-text { display: flex; flex-direction: column; line-height: 1.2; }
    .ec-devbadge-name { font-size: 11.5px; font-weight: 700; color: var(--text-hi); font-family: 'Outfit', sans-serif; transition: color .3s ease; }
    .ec-devbadge-sub { font-size: 10px; color: var(--text-lo); transition: color .3s ease; }
    .ec-devbadge:hover {
      background: linear-gradient(90deg, #833ab4, #fd1d1d, #fcb045, #833ab4);
      background-size: 300% auto;
      animation: ecInstaGradient 3s ease infinite;
      border-color: transparent;
      transform: translateY(-3px);
      box-shadow: 0 14px 32px rgba(253,29,89,.35);
    }
    .ec-devbadge:hover .ec-devbadge-name,
    .ec-devbadge:hover .ec-devbadge-sub { color: #fff; }
    .ec-devbadge:hover .ec-devbadge-icon { background: rgba(255,255,255,.28); }
    @keyframes ecInstaGradient {
...
function DevBadge() {
  return (
    ...
    >
      <span className="ec-devbadge-icon"><Instagram size={13} color="#fff" /></span>
  ...
  if (loading || !data) {
    return (
      <div className="ec-root" style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh" }}>
        <div style={{ color: "#e8cda0", fontFamily: "'Outfit', sans-serif", fontWeight: 700, fontSize: 20, letterSpacing: "-.02em" }}>
      ...
      <div className="ec-shell">
        ...
        <main className="ec-main" style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 16 }}>
          <header className="ec-header-glass">
            ...
            <div className="ec-serif" style={{ fontSize: 21, color: "var(--text-hi)" }}>{activeItem?.label}</div>
