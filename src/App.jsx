...
const GlobalStyle = () => (
  <style>{`
    ...
    .ec-serif { font-family: 'Fraunces', serif; font-weight: 500; letter-spacing: -.01em; }

    .ec-fade { animation: ecFadeIn .4s ease both; }
    ...
    .ec-card {
      ...
      border-radius: 16px;
      box-shadow: 0 4px 28px rgba(0,0,0,.28), inset 0 1px 0 rgba(255,255,255,.05);
      transition: border-color .2s ease, background .2s ease;
    }
    .ec-card:hover { background: rgba(255,255,255,.05); border-color: rgba(255,255,255,.14); }
    .ec-card.clickable { cursor: pointer; }
    .ec-card.clickable:hover { border-color: var(--gold); }
    ...
    .ec-navlink:hover { background: rgba(255,255,255,.05); color: var(--text-hi); }
    .ec-navlink.active {
    ...
    .ec-btn:hover { border-color: rgba(255,255,255,.18); background: rgba(255,255,255,.07); }
    ...
    .ec-btn.danger { border-color: rgba(255,255,255,.08); color: #c98a8a; }
    .ec-btn.danger:hover { background: rgba(155,60,60,.25); border-color: #6b3535; color: #eaa; }
    .ec-btn.ghost { border-color: rgba(255,255,255,.08); color: var(--text-lo); }
    .ec-btn.ghost:hover { background: rgba(255,255,255,.06); color: var(--text-hi); }

    .ec-input {
      width: 100%; background: rgba(255,255,255,.03); border: 1px solid rgba(255,255,255,.08);
      color: var(--text-hi); border-radius: 10px; padding: 9px 12px;
      font-size: 13.5px; outline: none; transition: border-color .15s ease;
      backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px);
    }
    .ec-input:focus { border-color: var(--gold); }
    .ec-label { font-size: 11px; text-transform: uppercase; letter-spacing: .7px; color: var(--text-lo); margin-bottom: 5px; display: block; }
    ...
    .ec-badge {
      ...
      color: var(--text-lo); background: rgba(255,255,255,.05);
    ...
    .ec-table td { padding: 10px 12px; border-bottom: 1px solid var(--line); color: var(--text-hi); }
    .ec-table tr:hover td { background: rgba(255,255,255,.03); }
    ...
    .ec-crest {
      ...
      color: var(--gold-soft); font-weight: 500; font-family: 'Fraunces', serif;
      font-size: 17px;
      flex-shrink: 0; overflow: hidden;
    }
    .ec-crest img { width: 100%; height: 100%; object-fit: cover; }

    .ec-orbs { position: fixed; inset: 0; overflow: hidden; z-index: 0; pointer-events: none; }
    .ec-orb { position: absolute; border-radius: 50%; filter: blur(100px); }
    .ec-orb-a { width: 460px; height: 460px; top: -140px; left: -120px; background: radial-gradient(circle, rgba(173,139,92,.30) 0%, transparent 70%); }
    .ec-orb-b { width: 520px; height: 520px; bottom: -180px; right: -140px; background: radial-gradient(circle, rgba(60,92,102,.30) 0%, transparent 70%); }
    .ec-orb-c { width: 380px; height: 380px; top: 40%; left: 50%; background: radial-gradient(circle, rgba(90,70,110,.18) 0%, transparent 70%); }
    ...
    .ec-hero-glow {
      ...
      background: radial-gradient(500px 260px at 15% 0%, rgba(173,139,92,.06), transparent 65%);
    }
    ...
    @media (max-width: 860px) {
      ...
      .ec-sidebar { position: fixed; z-index: 40; left: 0; top: 0; bottom: 0; height: 100vh; width: 260px; border-radius: 0; transform: translateX(-100%); transition: transform .3s ease; }
      .ec-sidebar.open { transform: translateX(0); }
      .ec-overlay { position: fixed; inset: 0; background: rgba(0,0,0,.5); z-index: 30; backdrop-filter: blur(2px); }
  ...
  if (loading || !data) {
    return (
      <div className="ec-root" style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh" }}>
        <div style={{ color: "#cbae85", fontFamily: "'Fraunces', serif", fontSize: 20, letterSpacing: .2 }}>
      ...
      <div className="ec-shell">
        ...
        <aside className={`ec-sidebar${sidebarOpen ? " open" : ""}`}>
          ...
          <div style={{ marginTop: "auto", paddingTop: 16 }}>
            ...
            <div style={{ fontSize: 9.5, color: "var(--text-lo)", lineHeight: 1.5, marginTop: 8, opacity: 0.7 }}>
              Dev by Daffa Mufti ·{" "}
              <a href="https://instagram.com/xnnzooo.id" target="_blank" rel="noreferrer" style={{ color: "var(--gold-soft)", textDecoration: "none" }}>
                @xnnzooo.id
              </a>
            </div>
          </div>
